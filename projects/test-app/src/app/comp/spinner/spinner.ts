...
export class Spinner {
	...
	simulate(which: 1 | 2 | 3, ms = 2000): void {
		if (which === 1) { this.btn1Loading.set(true); setTimeout(() => { this.btn1Loading.set(false); this.toastService.success('Saved!'); }, ms); }
		if (which === 2) { this.btn2Loading.set(true); setTimeout(() => { this.btn2Loading.set(false); this.toastService.info('Uploaded!'); }, ms); }
		if (which === 3) { this.btn3Loading.set(true); setTimeout(() => { this.btn3Loading.set(false); this.toastService.error('Failed — try again'); }, ms); }
	}
