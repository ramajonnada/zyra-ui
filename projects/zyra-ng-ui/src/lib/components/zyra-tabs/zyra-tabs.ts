...
@Component({
    ...
    providers: [
        { provide: ZYRA_TABS, useExisting: forwardRef(() => ZyraTabs) },
    ],
    host: {
        '[class]': '"zyr-tabs zyr-tabs--" + size() + " zyr-tabs--" + variant() + (orientation() === "vertical" ? " zyr-tabs--vertical" : "")',
    },
...
export class ZyraTabs implements ZyraTabsRef, AfterContentInit, AfterViewInit {
    ...
    private _syncIndicator(tab: ZyraTab): void {
        ...
        const trigger = this._el.nativeElement.querySelector(`#${tab.triggerId}`) as HTMLElement | null;
