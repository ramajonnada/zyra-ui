...
class ButtonHostComponent {
    variant = signal<'primary' | 'secondary' | 'ghost' | 'danger' | 'outline' | undefined>('primary');
    size = signal<'sm' | 'md' | 'lg' | undefined>('md');
    color = signal<'neutral' | 'accent' | 'success' | 'warning' | 'danger' | 'info' | undefined>(
        undefined,
    );
    radius = signal<'none' | 'sm' | 'md' | 'lg' | 'full' | undefined>(undefined);
...
describe('ZyraButton', () => {
    ...
    // ── Color ─────────────────────────────────────────────────────────────
    it('applies no color class by default', () => {
        expect(fixture.nativeElement.querySelector('[class*="zyr-btn--color-"]')).toBeNull();
    });

    it('applies a color modifier class when set', () => {
        host.color.set('success');
        fixture.detectChanges();
        expect(fixture.nativeElement.querySelector('.zyr-btn--color-success')).not.toBeNull();
    });

    it('does not apply a color class for "neutral"', () => {
        host.color.set('neutral');
        fixture.detectChanges();
        expect(fixture.nativeElement.querySelector('[class*="zyr-btn--color-"]')).toBeNull();
    });

    // ── Radius ────────────────────────────────────────────────────────────
    it('applies no radius modifier class by default', () => {
        expect(fixture.nativeElement.querySelector('[class*="zyr-btn--radius-"]')).toBeNull();
    });

    it('applies a radius modifier class when set', () => {
        host.radius.set('full');
        fixture.detectChanges();
        expect(fixture.nativeElement.querySelector('.zyr-btn--radius-full')).not.toBeNull();
    });

    // ── Standalone toggle (selected) ─────────────────────────────────────
    it('does not toggle selected on click unless toggleable', () => {
        fixture.nativeElement.querySelector('button').click();
        fixture.detectChanges();
        expect(fixture.nativeElement.querySelector('.zyr-btn--selected')).toBeNull();
        expect(fixture.nativeElement.querySelector('button').getAttribute('aria-pressed')).toBeNull();
    });

    it('toggles selected and aria-pressed on click when toggleable', () => {
        ...
        btn.click();
        fixture.detectChanges();
        expect(btn.classList).toContain('zyr-btn--selected');
        expect(btn.getAttribute('aria-pressed')).toBe('true');

        btn.click();
        fixture.detectChanges();
        expect(btn.classList).not.toContain('zyr-btn--selected');
