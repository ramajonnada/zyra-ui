...
- New semantic token `--zyra-color-surface-code`, routing `zyra-code-block`'s background through the semantic layer instead of a raw per-theme token.
- New semantic `--zyra-color-purple-*` tokens (foreground/subtle/border-color), giving Badge and Chip's `purple` variant a real place in the token-tier system instead of reaching straight past it.
- New Tier 3 component tokens: `zyra-chip` (full color-variant set, selected/selectable states, `purple`), `zyra-badge` (`purple`), `zyra-code-block` (background, border, header background), `zyra-toast` (per-variant icon bg/text, paired with `on-*` tokens), `zyra-modal` (`--zyra-modal-shadow`, no longer borrowing Card's).
...
- `zyra-modal` and `zyra-drawer` no longer borrow Card's `--zyra-card-elevated-shadow` Tier 3 token; each now has its own shadow token.
- Focus rings on `zyra-accordion`, `zyra-header` (mobile burger button), `zyra-pagination`, `zyra-sidebar`, `zyra-stepper`, `zyra-tabs`, `zyra-drawer`, and `zyra-modal` now use the shared `--zyra-ring` focus token instead of hardcoding `--zyra-color-primary`.
- `zyra-toast`'s colored icon variants (success/danger/warning/info) now pair a filled icon background with the matching `on-*` text token, consistent with the same fix already applied to `zyra-alert`.
- `zyra-stepper`'s completed-step indicator now uses `--zyra-color-on-brand` instead of the generic `--zyra-color-foreground-inverse`.
- `--zyra-color-btn-danger-hover-text` now uses `--zyra-color-on-danger` instead of the generic `--zyra-color-foreground-inverse`.
- **`ZyraButtonGroup`**: `equalWidth` (and `join="attached"` by default) didn't actually equalize button widths — `flex: 1 1 0` is a no-op in a shrink-to-fit container, so buttons rendered at their own text width. Switched to a CSS Grid equal-`fr` technique, which correctly equalizes without needing to stretch the group to 100% width.
- **`ZyraGrid`**: items placed in implicit tracks (e.g. via `autoFlow="column"` with more items than explicit columns) collapsed to their own content width instead of matching the explicit tracks' equal `1fr` share. `grid-auto-columns`/`grid-auto-rows` now mirror the explicit template's sizing.
- Published npm package now bundles the `LICENSE` file (previously omitted despite `package.json` declaring `"license": "MIT"`).

### Accessibility

- Theme-tokens doc page: swatch color chips are now `aria-hidden` instead of carrying a redundant/unreliable `aria-label` on a plain `<span>`.
- Theming doc page: the theme switcher (`role="radiogroup"`) now supports arrow-key navigation with roving `tabindex`, matching the expected native radiogroup keyboard pattern.

Components intentionally left unchanged: `accent-secondary` (Avatar gradients, Spinner's second color) still has no Tier 2 semantic alias, and `zyra-toast`/`zyra-tooltip`/`zyra-card`'s shadow/background tokens are deliberately tuned per-theme values consumed directly (documented in `_tokens-components.scss`) — both are pre-existing, intentional patterns, not bugs.
